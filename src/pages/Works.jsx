  ...
  return (
    <div className="p-6 text-foreground bg-background min-h-screen">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {filtered.map(p => (
          <article key={p.id} className="rounded-xl border border-border overflow-hidden bg-card shadow-sm">
            <div className="aspect-video bg-muted" />
            <div className="p-3">
              <div className="text-sm text-foreground/60">{p.year} · {p.category.toUpperCase()}</div>
              <h2 className="font-medium">{p.title}</h2>
              <p className="text-sm text-foreground/80 mt-1">{p.summary}</p>
              <div className="flex flex-wrap gap-1 mt-2">
                {p.tech.map(t => (
                  <span key={t} className="text-[10px] px-2 py-0.5 rounded bg-muted text-foreground/80 border border-border">
                    {t}
                  </span>
                ))}
              </div>
